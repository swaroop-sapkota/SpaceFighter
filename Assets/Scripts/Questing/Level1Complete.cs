...
public class Level1Complete : MonoBehaviour
{
    ...
    private void OnTriggerStay(Collider other)
    {
        if (other.CompareTag("Player"))
        {
            ...
            if (!(GetKeyObjective.material1Found && GetKeyObjective.material2Found && GetKeyObjective.material3Found))
            {
                GetKeyObj_Txt.SetActive(true);
            }
            else if (GetKeyObjective.material1Found && GetKeyObjective.material2Found && GetKeyObjective.material3Found && !unlockedNewLevel)
            {
